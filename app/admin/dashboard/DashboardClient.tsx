...
export function DashboardClient({ responses, totalInstitutions }: DashboardClientProps) {
  ...
  const regionPerformance = useMemo(() => {
    const map = new Map<string, { totalScore: number, count: number }>();
    filteredResponses.forEach(r => {
      const name = r.kabupaten_kota || "Unknown";
      if (!map.has(name)) map.set(name, { totalScore: 0, count: 0 });
      map.get(name)!.totalScore += (r.overall_score || 0);
      map.get(name)!.count += 1;
    });
    ...
    <div className="min-h-screen bg-white font-sans text-neutral-900 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
        ...
        <div>
          ...
          <div className="overflow-x-auto border border-neutral-200 rounded-3xl p-1">
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-neutral-50 text-neutral-500 text-xs uppercase tracking-wider">
                <tr>
                  <th className="py-4 px-5 font-medium rounded-tl-2xl">Rank</th>
                  <th className="py-4 px-5 font-medium">Instansi</th>
                  <th className="py-4 px-5 font-medium cursor-pointer hover:text-neutral-900 transition-colors" onClick={() => handleInstSort('avg')}>
                    <div className="flex items-center gap-1">
                      Nilai
                      {instSortConfig.key === 'avg' && (instSortConfig.direction === 'desc' ? <ChevronDown className="w-3 h-3" /> : <ChevronUp className="w-3 h-3" />)}
                    </div>
                  </th>
                  <th className="py-4 px-5 font-medium rounded-tr-2xl cursor-pointer hover:text-neutral-900 transition-colors" onClick={() => handleInstSort('count')}>
                    <div className="flex items-center gap-1">
                      Total
              ...
              <tbody className="divide-y divide-neutral-100">
                {instRanking.slice(0, 8).map((inst, idx) => (
        ...
        <div>
          <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
            <div>
              ...
              <p className="text-sm text-neutral-500 mt-1">Performa layanan berdasarkan kota/kabupaten.</p>
            </div>
            <div className="relative w-full sm:w-64">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400" />
              <input 
          ...
          <div className="overflow-x-auto border border-neutral-200 rounded-3xl p-1">
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-neutral-50 text-neutral-500 text-xs uppercase tracking-wider">
                <tr>
                  <th className="py-4 px-5 font-medium rounded-tl-2xl">Rank</th>
                  <th className="py-4 px-5 font-medium">Wilayah</th>
                  <th className="py-4 px-5 font-medium">Nilai</th>
                  <th className="py-4 px-5 font-medium rounded-tr-2xl">Total</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-neutral-100">
                {regionPerformance.slice(0, 8).map((reg, idx) => (
      ...
      <div>
        ...
        <div className="overflow-x-auto border border-neutral-200 rounded-3xl p-1">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-neutral-50 text-neutral-500 text-xs uppercase tracking-wider">
              <tr>
                <th className="py-4 px-6 font-medium rounded-tl-2xl">Tanggal</th>
                <th className="py-4 px-6 font-medium">Penilai</th>
                <th className="py-4 px-6 font-medium">Instansi Dinilai</th>
                <th className="py-4 px-6 font-medium rounded-tr-2xl">Skor</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-neutral-100">
              {recentAssessments.map(r => (
                <tr key={r.id} className="hover:bg-neutral-50/50 transition-colors">
                  ...
                  <td className="py-4 px-6 font-medium text-neutral-900">{r.nama_penilai}</td>
                  <td className="py-4 px-6 text-neutral-600 truncate max-w-[200px]" title={r.institutions?.name}>{r.institutions?.name || "-"}</td>
