...
import { Users, Star, Building2, AlertCircle, ArrowUpRight, MessageSquare } from "lucide-react";
...
interface DashboardClientProps {
  ...
  trendData: { name: string; responses: number }[];
  recentSubmissions: {
    id: string;
    inst: string;
    date: string;
  ...
  topComplaints: { text: string; inst: string; date: string }[];
  topSuggestions: { text: string; inst: string; date: string }[];
}

export function DashboardClient({ stats, trendData, recentSubmissions, topComplaints, topSuggestions }: DashboardClientProps) {
  return (
    <div className="p-8">
      <div className="mb-10">
        <h1 className="text-3xl font-bold text-neutral-950 mb-2 tracking-tight">Dashboard Overview</h1>
        <p className="text-neutral-500">Ringkasan performa dan metrik kepuasan pelayanan publik dari database.</p>
      </div>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
          {[
            { label: "Total Responden", value: stats.totalResponses, formatter: formatNumber, icon: Users, trend: "+-" },
            { label: "Rata-rata Kepuasan", value: stats.averageScore * 10, formatter: (v: number) => (v/10).toFixed(1), icon: Star, trend: "+-" },
            { label: "Instansi Aktif", value: stats.activeInstitutions, formatter: formatNumber, icon: Building2, trend: "+-" },
            { label: "Perlu Perbaikan", value: stats.needsImprovement, formatter: (v: number) => v.toString(), icon: AlertCircle, trend: "+-" },
          ].map((stat, i) => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <motion.div variants={FADE_UP} className="lg:col-span-2 p-6 bg-white border border-neutral-200 rounded-xl shadow-sm">
            <h3 className="text-lg font-semibold text-neutral-950 mb-6">Tren Partisipasi (7 Hari Terakhir)</h3>
            ...
            <div className="flex-1 overflow-y-auto pr-2">
              <div className="space-y-4">
                ...
                {recentSubmissions.length === 0 && (
                  <p className="text-sm text-neutral-400 text-center py-4">Belum ada data.</p>
                )}
              </div>
            </div>
          </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
          ...
          <motion.div variants={FADE_UP} className="p-6 bg-white border border-neutral-200 rounded-xl shadow-sm flex flex-col">
            <div className="flex items-center gap-2 mb-6 text-red-600">
              ...
              <h3 className="text-lg font-semibold text-neutral-950">Sorotan Keluhan</h3>
            </div>
            <div className="flex-1 overflow-y-auto max-h-[300px] pr-2 space-y-4">
              {topComplaints.map((comp, i) => (
                <div key={i} className="p-4 bg-red-50/50 rounded-lg border border-red-100">
                  <p className="text-neutral-700 text-sm mb-3 italic">"{comp.text}"</p>
              ...
              {topComplaints.length === 0 && (
                <p className="text-sm text-neutral-400 text-center py-4">Belum ada keluhan yang masuk.</p>
              )}
            </div>
          </motion.div>
          ...
          <motion.div variants={FADE_UP} className="p-6 bg-white border border-neutral-200 rounded-xl shadow-sm flex flex-col">
            <div className="flex items-center gap-2 mb-6 text-blue-600">
              ...
              <h3 className="text-lg font-semibold text-neutral-950">Sorotan Saran</h3>
            </div>
            <div className="flex-1 overflow-y-auto max-h-[300px] pr-2 space-y-4">
              {topSuggestions.map((sug, i) => (
                <div key={i} className="p-4 bg-blue-50/50 rounded-lg border border-blue-100">
                  <p className="text-neutral-700 text-sm mb-3 italic">"{sug.text}"</p>
                  <div className="flex justify-between items-center text-xs text-neutral-500 font-medium">
                    <span>{sug.inst}</span>
                    <span>{sug.date}</span>
                  </div>
              ...
              {topSuggestions.length === 0 && (
                <p className="text-sm text-neutral-400 text-center py-4">Belum ada saran yang masuk.</p>
              )}
            </div>
          </motion.div>
